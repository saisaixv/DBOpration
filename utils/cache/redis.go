...
func newPool(url string,max_idle,idle_timeout int,db int) *redis.Pool {
	...
	return &redis.Pool{
		...
		Dial:func () (redis.Conn,error) {
			...
			if err!=nil{
				fmt.Println("Dial redis select db error")
				fmt.Println(err)
			}
		...
		TestOnBorrow:func (c redis.Conn, t time.Time) error {
			if time.Since(t)<time.Minute{
				fmt.Println("TestOnBorrow redis select db error")
				return nil
			}
...
//共享锁
//-----------------------------------------------------------
//lockStart开始一个分布式锁，retock:是否锁成功，尝试n次，每次间隔100毫秒
...
//lockHeart锁的心跳，锁的超时时间很短，一旦没有心跳，锁就自动解锁
